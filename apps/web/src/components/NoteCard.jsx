...
export default function NoteCard({ note, variant = "grid", tone = "soft" }) {
  ...
  const bgGrid = archived
    ? "#fbfbfc"
    : status === "resuelta"
    ? RESOLVED_BG
    : pastelize(toHex, tone === "pro" ? 0.18 : 0.26);
  const bgList =
    status === "resuelta" ? RESOLVED_BG : pastelize(toHex, 0.12);

  const dState = dueState(dueAt, { archived, status });
  const hasReplies = Array.isArray(replies) && replies.length > 0;
  const lastReplyAuthor = hasReplies
    ? replies[replies.length - 1].author
    : null;
    ...
    if (abs >= THRESHOLD) {
      if (dx > 0) {
        ...
        notify({
          ...
          title:
            next === "resuelta" ? "Marcada como resuelta" : "Marcada como pendiente",
        });
      } else {
        if (archived) {
          unarchiveNote(id);
          notify({ variant: "info", title: "Nota desarchivada" });
        } else {
          archiveNote(id);
          notify({ variant: "info", title: "Nota archivada" });
        }
      }
      setOffset(0);
      setDragSide(null);
  ...
  if (variant === "list") {
    ...
    return (
      <TooltipProvider
        delayDuration={isTouch ? 0 : 150}
        disableHoverableContent={isTouch}
      >
        <div
        ...
        >
          {status === "resuelta" && (
            ...
            >
              <CheckCircle2
                className="w-10 h-10 sm:w-11 sm:h-11 opacity-15"
                style={{ color: "#059669" }}
              />
            </div>
          )}
            ...
            <div className="absolute inset-y-0 left-0 w-full pr-16 grid place-items-center justify-start">
              ...
              >
                ...
                <span className="text-[13px] font-medium">
                  {status === "pendiente" ? "Resuelta" : "Pendiente"}
                </span>
              </div>
            </div>
          ...
          <motion.div
            role="button"
            tabIndex={0}
            aria-label={`Nota de ${from || "—"} para ${to || "—"}. ${
              text || "Sin texto"
            }`}
            ...
            whileHover={
              prefersReducedMotion ? undefined : { y: -1, scale: 1.002 }
            }
            ...
            <div className="relative z-[1] h-full grid grid-cols-[auto,1fr,auto] items-center gap-2 sm:gap-3 px-2.5 sm:px-3">
              <div className="flex items-center gap-1.5 sm:gap-2 max-w-[42%] sm:max-w-[260px]">
                <span
                  ...
                  style={{
                    color: pastelize(fromHex, 1),
                    borderColor: pastelize(fromHex, 0.55),
                  }}
                  title={`De: ${from || "—"}`}
                >
                  {from || "—"}
                </span>
                <span className="text-[10.5px] sm:text-[12px] text-slate-500">
                  →
                </span>
                <span
                  className="text-[10.5px] sm:text-[12px] font-semibold px-1 py-0.5 sm:px-1.5 rounded-full border bg-white/90 backdrop-blur truncate"
                  style={{
                    color: pastelize(toHex, 1),
                    borderColor: pastelize(toHex, 0.55),
                  }}
                  title={`Para: ${to || "—"}`}
                >
                  {to || "—"}
              ...
              <p
                className="text-[12.5px] sm:text-[13px] text-slate-800 min-w-0 truncate"
              ...
              <div className="flex items-center gap-1.25 sm:gap-2 pl-1.5 sm:pl-2 shrink-0">
                {dueAt && !archived && (
                  <span
                    className={cn(
                      ...
                      dueState(dueAt, { archived, status }) === "overdue" &&
                        "bg-rose-50 text-rose-700 border-rose-200",
                      dueState(dueAt, { archived, status }) === "soon" &&
                        "bg-amber-50 text-amber-700 border-amber-200",
                      dueState(dueAt, { archived, status }) === "future" &&
                        "bg-white/85 text-slate-700 border-slate-200"
                    )}
                ...
                {deleted && (
                  <TooltipProvider
                    delayDuration={useIsTouchDevice() ? 0 : 150}
                    disableHoverableContent={useIsTouchDevice()}
                  >
                    <Tooltip>
    ...
    >
      ...
      >
        {status === "resuelta" && (
          ...
          >
            <CheckCircle2
              className="w-24 h-24 sm:w-28 sm:h-28 opacity-15"
              style={{ color: "#059669" }}
            />
          </div>
        )}
        ...
        <div className="px-3 py-2 border-t border-slate-300/40 bg-white/40 backdrop-blur-[1px] text-[11px] text-slate-650 relative z-10">
          <div className="flex items-center justify-between">
            ...
            {!deleted ? (
              <div className="flex gap-2 items-center">
                <WrapTT
                  label={status === "pendiente" ? "Marcar resuelta" : "Marcar pendiente"}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <span
                className={cn(
                  ...
                  dState === "future" &&
                    "bg-white/85 text-slate-700 border-slate-200",
                  dState === "none" &&
                    "bg-white/70 text-slate-600 border-slate-200/70"
                )}
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-2">
            ...
            {hasReplies && (
              ...
              >
                <MessageSquareText className="w-3.5 h-3.5" />
                {replies.length} respuesta{replies.length > 1 ? "s" : ""} · Últ.:{" "}
                {lastReplyAuthor}
              </span>
            )}
          </div>
